...
public:
	...
	*/
	void loadTexture()
	{
        ...
        printf("Loaded texture [%d x %d] w/ %d MIP levels. Size: %zd bytes.\n", texture.width, 
                                                                                texture.height, 
                                                                                texture.mipLevels,
                                                                                ktxTextureSize);
        printf("\tPixel vals = [");
        for (size_t ii = 0; ii < 12; ii++) {
            printf("%02x ", *(ktxTextureData + ii));
        }
        printf("]\n");
    ...
    void loadTextureFromFile(std::string fname)
	{
		...
		if (!vks::tools::fileExists(texture_filename)) {
			vks::tools::exitFatal("Could not load texture from " + texture_filename + "\n\nMake sure the assets submodule has been checked out and is up-to-date.", -1);
		}
        ...
        printf("Loaded texture [%d x %d] w/ %d channels and %d MIP levels. Size: %zd\n", texture.width, 
                                                                               texture.height, 
                                                                               tex_channels,
                                                                               texture.mipLevels,
                                                                               image_size);
		...
		if (useStaging) {
            ...
            // Setup a buffer image copy structure for the only mip level
            VkBufferImageCopy bufferCopyRegion = {};
            bufferCopyRegion.imageSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
