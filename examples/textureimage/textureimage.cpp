...
public:
	...
	std::string toString() const {
		char buf[256];
		snprintf(buf, 256, "P: {%6.3f, %6.3f, %6.3f} T: {%6.3f, %6.3f} N: {%6.3f, %6.3f, %6.3f}",
			pos[0], pos[1], pos[2], uv[0], uv[1], normal[0], normal[1], normal[2]);
...
public:
	...
	void mapTextureToVertex(float s, float t, float& u, float& v) {
		float s_norm = 2.0f * s - 1.0f;	// Scale from -1 to 1
		float t_norm = 2.0f * t - 1.0f;	

		// Use nonlinear camera model
		...
		double k1 = -0.32989944290415379;
		double k2 =  0.091142595057391626;
		double k3 = -0.00074268896306007443;
		double k4 =  0.00064894864357395617;
		double k5 = -0.00349843889391524;

		double k_param = (1 + k1*r2 + k2*r4 + k5*r6);

		double dx_dist, dy_dist;
		dx_dist = 2*k3*dx*dy+k4*(r2 + 2*dx*dx);
		dy_dist = 2*k4*dx*dy+k3*(r2 + 2*dy*dy);

		u = k_param * dx + dx_dist;
		v = k_param * dy + dy_dist;
	}
	...
	*/
	void loadTextureFromImage() {
		// Load texture data into VkImage using the stbi library
		int texWidth, texHeight, texChannels;
		std::string texture_filename = getAssetPath() + "textures/statue.jpg";
		stbi_uc* pixels = stbi_load(texture_filename.c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void generateQuad(size_t nvert)
	{
		// Build evenly-spaced vertex array
		std::vector<Vertex> vertices;

		for (size_t row = 0; row < nvert; row++) {
			for (size_t col = 0; col < nvert; col++) {
				float s = (float) col / ((float) nvert - 1.0f);
				float t = (float) row / ((float) nvert - 1.0f);

				float u,v;
				mapTextureToVertex(s, t, u, v);
				Vertex new_vertex;
				new_vertex.pos[0] = u;
				new_vertex.pos[1] = v;
				new_vertex.pos[2] = 0.0f;
				
				new_vertex.normal[0] = 0.0f;
				new_vertex.normal[1] = 0.0f;
				new_vertex.normal[2] = 1.0f;
				...
				std::cout << new_vertex.toString() << "\n";
			}
		}

		// Setup indices
		std::vector<uint32_t> indices;
		indices.reserve(2*(nvert - 1)*(nvert - 1));
		
		for (size_t row = 0; row < (nvert - 1); row++) {
			for (size_t col = 0; col < (nvert - 1); col++) {
				// Add two clockwise triangles

				uint32_t idx_start = row * nvert + col;
				uint32_t va = idx_start + 1 + nvert;
				uint32_t vb = idx_start + nvert;
				uint32_t vc = idx_start;
				uint32_t vd = idx_start + 1;
